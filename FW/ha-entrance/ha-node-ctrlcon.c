...
void ctrlcon_on_rx(uint8_t idx, const uint8_t *buf_in) 
{
    ...
    if (buf_in[NLINK_HDR_OFF_CMD] != NLINK_CMD_RD_RESP) return;
    ...
    // Copy data from received packet to newly created peer node and process it as just received from peer
